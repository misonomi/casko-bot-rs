...
pub fn watch(user: &User) {
    match add_meltomo(&user.id) {
        Some(_) => dm_facade(user, "Now I'm watching you!"),
        None => dm_facade(user, "I'm already watching you."),
    }
}
...
pub fn unwatch(user: &User) {
    match remove_meltomo(&user.id) {
        Some(_) => dm_facade(user, "I'm no longer watching you."),
        None => dm_facade(user, "I'm not watching you."),
    }
}

pub fn status(msg: &Message) {
    match has_meltomo(&msg.author.id) {
...
pub fn stat_update(game: Option<&Game>, player: &Meltomo) {
    ...
    match (game, player.game_as_option()) {
        (Some(new_game), Some(old_game)) => {
            ...
            update_game(&user.id, Some(new_game.clone()));
        },
        (Some(new_game), None) => {
